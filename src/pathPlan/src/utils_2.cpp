...
vector<float> dist_a_point_to_points(const Eigen::Vector2f &point1, const vector<Eigen::Vector2f> &points2)
{
...
Eigen::Vector2f rotation_center(Eigen::Vector2f point, Eigen::Vector2f center, float angle)
{
	...
	rotate[0] = (point[0]-center[0])*cos(angle)-(point[1]-center[1])*sin(angle)+center[0];
	rotate[1] = (point[0]-center[0])*sin(angle)+(point[1]-center[1])*cos(angle)+center[0];
	return rotate;
}
